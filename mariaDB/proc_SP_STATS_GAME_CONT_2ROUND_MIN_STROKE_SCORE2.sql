CREATE PROCEDURE  `SP_STATS_GAME_CONT_2ROUND_MIN_STROKE_SCORE`(
IN $GM_CODE CHAR(13),
IN $MODE VARCHAR(20)
)
BEGIN
  IF $MODE is NULL THEN
    ...
    AND STATS_DIV = '2ROUND_MIN_STROKE';
  ELSEIF $MODE = 'GEN' THEN
    BEGIN
      ...
      DROP TEMPORARY TABLE IF EXISTS _TMP3;
      CREATE TEMPORARY TABLE IF NOT EXISTS _TMP3 (
      	SUM_STROKE INT DEFAULT '0' COMMENT '타수 집계',
      	MM_CODE VARCHAR(8) COMMENT '회원코드',
      	MM_NAME_DISPLAY VARCHAR(60) COMMENT '이름',
      	E_MM_NAME_DISPLAY VARCHAR(60) COMMENT '영문이름'
      ) COMMENT='3~4R 최저타 집계 임시 테이블';
      ...
      SELECT MIN(SUM_STROKE) INTO $MIN_STROKE3 FROM _TMP3 WHERE SUM_STROKE > 0;

      INSERT INTO _TMP
      SELECT SUM_STROKE, MM_CODE, MM_NAME_DISPLAY, E_MM_NAME_DISPLAY
      FROM _TMP1
      WHERE SUM_STROKE = $MIN_STROKE1;

      INSERT INTO _TMP
      SELECT SUM_STROKE, MM_CODE, MM_NAME_DISPLAY, E_MM_NAME_DISPLAY
      FROM _TMP2
      WHERE SUM_STROKE = $MIN_STROKE2;

      INSERT INTO _TMP
      SELECT SUM_STROKE, MM_CODE, MM_NAME_DISPLAY, E_MM_NAME_DISPLAY
      FROM _TMP3
      WHERE SUM_STROKE = $MIN_STROKE3;
      ...
      SELECT $GM_CODE, MM_CODE, '2ROUND_MIN_STROKE',MM_NAME_DISPLAY, E_MM_NAME_DISPLAY, SUM_STROKE
