...
BEGIN
  ...
  ELSEIF $MODE = 'GEN' THEN
    BEGIN
      ...
      CREATE INDEX _TMP2_IDX2 ON _TMP(CONT_BIRDIE);
      ...
      SELECT MAX(CONT_BIRDIE),MAX(CONT_PAR),MAX(CONT_NOBOGEY)
      INTO @MAX_BIRDIE, @MAX_PAR, @MAX_NOBOGEY FROM _TMP;

      INSERT INTO _TMP2 (
        MM_CODE, MM_NAME_DISPLAY, E_MM_NAME_DISPLAY,
        CONT_BIRDIE, CONT_PAR, CONT_NOBOGEY
      )
      SELECT MM_CODE, MM_NAME_DISPLAY, E_MM_NAME_DISPLAY,
        CONT_BIRDIE, CONT_PAR, CONT_NOBOGEY
      FROM _TMP
      WHERE
      CONT_BIRDIE = @MAX_BIRDIE
      OR CONT_PAR = @MAX_PAR
      OR CONT_NOBOGEY = @MAX_NOBOGEY;

      UPDATE _TMP2 SET CONT_BIRDIE = 0 WHERE CONT_BIRDIE < @MAX_BIRDIE;
      ...
      WHERE GM_CODE = $GM_CODE
      AND STATS_DIV IN ('CONT_BIRDIE','CONT_PAR','CONT_NOBOGEY');
